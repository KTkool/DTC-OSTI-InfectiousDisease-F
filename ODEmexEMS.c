...
#define odeInit initDerivativesEMS
#define odeFunction derivativesEMS
#define odeStepper rk4Step


void initDerivativesEMS(const mxArray *params)
{
    ...
    d = mxGetScalar(mxGetField(params,0,"d"));
    k = mxGetScalar(mxGetField(params,0,"k"));
    p = mxGetScalar(mxGetField(params,0,"p"));
    c = mxGetScalar(mxGetField(params,0,"c"));
    theta = mxGetScalar(mxGetField(params,0,"theta"));
}

void derivativesEMS(double t, const double* currenty, double* nexty)
{
    nexty[0]=s-d*currenty[0]-beta*currenty[0]*currenty[2];
    nexty[1]=beta*currenty[0]*currenty[2]-(alpha*d+k*currenty[1]/(currenty[1]+theta))*currenty[1];
